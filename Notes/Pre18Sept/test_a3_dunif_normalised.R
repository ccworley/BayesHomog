...
prior.matrix = (1/num.nodes)*diag(rgamma(num.nodes,0.1,0.1))  #(1/num.nodes)*diag((100/sd.bm.teff)^2,ncol=num.nodes,nrow=num.nodes) 
...
true.teff.benchmark.per.spec[i.of.missing.values[l],j.of.missing.values[l]] ~ dnorm(0,pow(1,-2))   #dnorm(5300,pow(800,-2))   #dunif(-1,1)   #dunif(3000,8000)   #dnorm(0,1) #when scaled by meanTbm and sigTbm  #
...
for (l in 1:K) {
  for (m in 1:P) {
    ...
    alpha3[l,m] ~ dunif(-1, -0.01)    #     dnorm(0,pow(sd.of.alpha3,-2))  #
...
inits <- function() {
  list(alpha1 = matrix(rnorm(num.setups*num.nodes,0,sd.of.alpha1),ncol=num.setups,nrow=num.nodes),    # Here, outside JAGS, rnorm actually uses mu and sd, the standard deviation.
       ...
       alpha3 = matrix(runif(num.setups*num.nodes,-1, -0.01),ncol=num.setups,nrow=num.nodes),   #matrix(rnorm(num.setups*num.nodes,0,sd.of.alpha3),ncol=num.setups,nrow=num.nodes),     #
       teff.InvCovMat = prior.matrix  #diag(rgamma(num.nodes,0.1,0.1))
  )
}
