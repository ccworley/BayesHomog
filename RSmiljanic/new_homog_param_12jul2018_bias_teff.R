...
# Now, here comes an important trick. There are many missing values (NA or NaN) in the measurements given by the nodes. Missing values need special treatment.
...
#6) We are going to fit one bias function per setup (i.e., U520, U580 - this can eventually be extended for Giraffe, but maybe not easilly. In WG11 all nodes analyze all spectra of all setups. That is not the case in WG10. This would break the symmetry of the data structures per setup). We create a vector with a code for each setup:

setups.used <- levels(as.factor((metadata.of.bench.spectra$SETUP)))    # Finds out all the setups. Below it converts each setup "name" to a nummeric code
