...
# Now, here comes an important trick. 
# There are many missing values (NA or NaN) in the measurements given by the nodes. Missing values need special treatment.
...
plot.against.reference.bench <- function(the.model,variable=c('TEFF'),bench.data=bench.param) {
  ...
  plotCI(bench.data$TEFF,(bench.data[,the.col]-test[,4]),main=variable,xlab=paste0('Given TEFF of Benchmarks'),ylab=paste0('Delta ',variable,' (given minus homog)'))
