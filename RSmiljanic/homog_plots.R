...
load.nodes <- function(list.nodes,release=this.release,where.files=node.files.path) {
  ...
  #
  list.files <- paste0(where.files,'GES_',substr(release,1,4),'_WG11_',list.nodes,'.fits')
  ...
  for (ik in seq(1,num.nodes,1)) {
    ...
    for (jk in seq(1,length(list.param.names),1)) {
      ...
      if (((list.nodes[ik] == 'Nice') || (list.nodes[ik] == 'IACAIP')) && ((list.param.names[jk] == 'FEH') || (list.param.names[jk] == 'E_FEH'))) {
...
######################################## 
#FIRST of all, correct IACAIP from the box grid results (teff > 7500 or < 3500 log g > 4.5 or < 0.5)
#
correct.iacaip.grid <- function(in.nodes.param) {
  ...
  #
  filter.col1 <- !is.na(selec.col) & ((selec.col >= 7500) | (selec.col <= 3500))
  #
  selec.col <- in.nodes.param[,col.iac,col.logg]
  selec.col <- as.data.frame(selec.col)
  selec.col <- apply(selec.col,2,as.numeric)
  #    
  filter.col2 <- !is.na(selec.col) & ((selec.col <= 0.5) | (selec.col >= 4.5))
  #
  final.filter <- filter.col1 | filter.col2
  #
