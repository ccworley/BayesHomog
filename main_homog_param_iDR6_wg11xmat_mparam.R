...
if ("HR15N" %in% anasetup) {
  list.nodes <- c('Lumba','OACT','EPINARBO')   #we,'MaxPlanck')
...
if ("HR10|HR21" %in% anasetup){
  ofsu <- paste('Output/',suffix,'/',paste("WG10",release,"HR10HR21",suffix,sep="_"),sep="")
} else {
  ofsu <- paste('Output/',suffix,'/',paste("WG10",release,anasetup,suffix,sep="_"),sep="")
}
...
flags.to.ignore.specific <- c("10302","10303","10304","10305","11020","13022", # EPINARBO
                              ...
                              "10302", #IAC
                              "10303","10305","10308","10399") # MaxPlanck
...
# iacflag = ['10302']  ; IAC
# maxflag = ['10303','10305','10308','10399']  ; MaxPlanck  ; Keep = '10020',;Added flag to remove MS flicked tail
...
source('models_rjags_homog_param_ncoeff.R')
...
#if ("HR15N" %in% anasetup) {
  filter.bench <- (bench.param$ID1 %in% nodes.ids$CNAME  & !is.na(bench.param$FEH))
#} else {
...
for (ik in seq(1,length(bench.param$GES_FLD))) {
  ...
  #mean.bench.snr[ik] <- min(snr.spec.vec[which(bench.param$GES_FLD == metadata.of.bench.spectra$GES_FLD[ik])])
}
...
#Making thse all the same for all three variables
...
# Create the manipulated data vectors that take care of missing values -  Teff and logg and feh

manipulated.data.teff.logg.feh <- create.manipulated.teff.logg.feh(observed.teff=observed.node.teff.spectrum,observed.logg=observed.node.logg.spectrum,observed.feh=observed.node.feh.spectrum,bench.data=bench.param,the.stars=star.code)
manipulated.node.teff.spectrum <- manipulated.data.teff.logg.feh[[1]]
manipulated.node.logg.spectrum <- manipulated.data.teff.logg.feh[[2]]
manipulated.node.feh.spectrum <- manipulated.data.teff.logg.feh[[3]]
i.of.missing.teff <- manipulated.data.teff.logg.feh[[4]]
j.of.missing.teff <- manipulated.data.teff.logg.feh[[5]]
i.not.missing.teff <- manipulated.data.teff.logg.feh[[6]]
j.not.missing.teff <- manipulated.data.teff.logg.feh[[7]]
code.bench.missing <- manipulated.data.teff.logg.feh[[8]]
code.bench.not.missing <- manipulated.data.teff.logg.feh[[9]]
keep.track.of.missing <- manipulated.data.teff.logg.feh[[10]]
num.missing.values <- manipulated.data.teff.logg.feh[[11]]
num.not.missing.values <- manipulated.data.teff.logg.feh[[12]]
rm(manipulated.data.teff.logg.feh)
...
# Number of coefficients: if function of 1 parameter=3, if f of 2 = 6, if f of 3 = 9
ncoeff <- 6  #3  #
...
teff.data <- list(given.teff.benchmarks = given.teff.bench,                      # Benchmark Teff
                  ...
                  given.feh.benchmarks = given.feh.bench,                       # Benchmark Teff
...
logg.data <- list(given.teff.benchmarks = given.teff.bench,                      # Benchmark Teff
                  given.error.teff.benchmarks = given.sigma.teff.bench,          # Benchmark e_Teff
                  observed.node.teff.per.spec = manipulated.node.teff.spectrum,  # Node Teff
                  given.logg.benchmarks = given.logg.bench,                      # Benchmark Teff
                  given.error.logg.benchmarks = given.sigma.logg.bench,          # Benchmark e_Teff
...
feh.data <- list(given.teff.benchmarks = given.teff.bench,                      # Benchmark Teff
                 given.error.teff.benchmarks = given.sigma.teff.bench,          # Benchmark e_Teff
                 observed.node.teff.per.spec = manipulated.node.teff.spectrum,  # Node Teff
                 given.logg.benchmarks = given.logg.bench,                      # Benchmark Teff
                 given.error.logg.benchmarks = given.sigma.logg.bench,          # Benchmark e_Teff
...
list.inits.teff <- create.inits.ncoeff(variable=c('TEFF'),num.chains=num.chains.for.teff,n.setups=num.setups,n.nodes=num.nodes,n.coeff=ncoeff)
list.inits.logg <- create.inits.ncoeff(variable=c('LOGG'),num.chains=num.chains.for.logg,n.setups=num.setups,n.nodes=num.nodes,n.coeff=ncoeff)
list.inits.feh <- create.inits.ncoeff(variable=c('FEH'),num.chains=num.chains.for.feh,n.setups=num.setups,n.nodes=num.nodes,n.coeff=ncoeff)

# Here are the functions that actually run the MCMC chains to create the reference model based on the benchmarks

model.for.teff <- run.the.reference.model(input.data=teff.data,the.model=model.teff.feh.matrix.bias,num.chains=num.chains.for.teff,num.adapt=1000,num.bur=1000,
...
model.for.logg <- run.the.reference.model(input.data=logg.data,the.model=model.logg.feh.matrix.bias,num.chains=num.chains.for.logg,num.adapt=1000,num.bur=1000,
...
model.for.feh <- run.the.reference.model(input.data=feh.data,the.model=model.feh.logg.matrix.bias,num.chains=num.chains.for.feh,num.adapt=1000,num.bur=1000,
...
plot.against.reference.bench(model.for.feh,c('FEH'),bench.param,mean.bench.snr)


#TEFF and LOGG
#look.at.node.bias.multifunctions(the.model=model.for.teff,variable=c('TEFF'),variable2=c('LOGG'),bench.data=bench.param,observed.data=observed.node.teff.spectrum,
...
#                            mean.param2.bench=mean(given.logg.bench,na.rm=T),sd.param2.bench=sd(given.logg.bench,na.rm=T),
#                            observed.snr=snr.spec.vec)

#TEFF and FEH
#look.at.node.bias.multifunctions(the.model=model.for.teff,variable=c('TEFF'),variable2=c('FEH'),bench.data=bench.param,observed.data=observed.node.teff.spectrum,
#                                 observed.data2=observed.node.feh.spectrum,n.coeff=ncoeff,
#                                 the.setups=vector.of.setups,the.stars=star.code,col.of.setups=metadata.of.bench.spectra$SETUP,
#                                 nodes=list.nodes,mean.param.bench=mean(given.teff.bench,na.rm=T),sd.param.bench=sd(given.teff.bench,na.rm=T),
#                                 mean.param2.bench=mean(given.feh.bench,na.rm=T),sd.param2.bench=sd(given.feh.bench,na.rm=T),
#                                 observed.snr=snr.spec.vec)

#LOGG and FEH
#look.at.node.bias.multifunctions(the.model=model.for.logg,variable=c('LOGG'),variable2=c('FEH'),bench.data=bench.param,observed.data=observed.node.logg.spectrum,
#                                 observed.data2=observed.node.feh.spectrum,n.coeff=ncoeff,
#                                 the.setups=vector.of.setups,the.stars=star.code,col.of.setups=metadata.of.bench.spectra$SETUP,
...
#data.for.ref.homog.teff <- prepare.for.full.sample.homog(variable='TEFF',this.model=model.for.teff,full.metadata=metadata.of.bench.spectra,all.param=node.measured.param.bench,bench.data=bench.param)
#data.for.ref.homog.logg <- prepare.for.full.sample.homog(variable='LOGG',this.model=model.for.logg,full.metadata=metadata.of.bench.spectra,all.param=node.measured.param.bench,bench.data=bench.param)
#data.for.ref.homog.feh <- prepare.for.full.sample.homog(variable='TEFF',this.model=model.for.feh,full.metadata=metadata.of.bench.spectra,all.param=node.measured.param.bench,bench.data=bench.param)

#ref.data.homog <- create.data.table(homog.teff=model.for.teff,homog.logg=model.for.teff,homog.feh=model.for.teff,metadata.here=data.for.ref.homog.feh)

#stop('Reference Models done')
# Save the models for future use

save(model.for.teff,file=paste(ofsu,'my_reference_teff_model.Rdata',sep="_"))
save(model.for.logg,file=paste(ofsu,'my_reference_logg_model.Rdata',sep="_"))
save(model.for.feh,file=paste(ofsu,'my_reference_feh_model.Rdata',sep="_"))
...
data.for.full.homog.teff <- prepare.for.full.sample.homog.mparam(variable='TEFF',variable2='FEH',this.model=model.for.teff,full.metadata=nodes.ids,all.param=clean.nodes.param,bench.data=bench.param)
data.for.full.homog.logg <- prepare.for.full.sample.homog.mparam(variable='LOGG',variable2='FEH',this.model=model.for.logg,full.metadata=nodes.ids,all.param=clean.nodes.param,bench.data=bench.param)
data.for.full.homog.feh <- prepare.for.full.sample.homog.mparam(variable='FEH',variable2='LOGG',this.model=model.for.feh,full.metadata=nodes.ids,all.param=clean.nodes.param,bench.data=bench.param)
...
save(full.teff.homog,file=paste(ofsu,'results_homog_teff_full_sample.Rdata',sep="_"))
save(full.teff.homog,file=paste(ofsu,'results_homog_logg_full_sample.Rdata',sep="_"))
save(full.teff.homog,file=paste(ofsu,'results_homog_feh_full_sample.Rdata',sep="_"))
...
write.table(all.data.homog,file=paste(ofsu,'.txt',sep=""),row.names=FALSE)
